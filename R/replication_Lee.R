...
sim_res_sample <- setNames(rep(list(vector("list", R)), length(dgp_settings)), dgp_settings)
#sim_res_pop <- setNames(rep(list(vector("list", R)), length(dgp_settings)), dgp_settings)
...
for (r in 1:R) { # loop level 1: R monte carlo iterations
  ...
  for (j in 1:length(dgp_settings)) { # loop level 2: data generating processes
    ...
    sample_size <- round(runif(D, min = 0, max = 30))
    sample_r <- sampler(DAT = pop_r, sample_size = sample_size, D = D)
    sim_res_sample[[dgp_settings[j]]][[r]] <- sample_r
    # sim_res_pop[[j]][[r]] <- pop_r
...
save(sim_res_sample, sim_res_JcAIC, file = "data/replication_R10_z_b50.RData")
